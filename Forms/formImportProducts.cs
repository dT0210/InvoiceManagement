...
{
    public partial class formImportProducts : Form
    {
        ...
        private void formImportProducts_Load(object sender, EventArgs e)
        {
            loadComboBox();
        }
        ...
        private void importButton_Click(object sender, EventArgs e)
        {
            using (SqlConnection connect = new SqlConnection(connectionString))
            {
                ...
                {
                    connect.Open();
                    queryString = "insert into ImportList (DateTime) values (@datetime)";
                    ...
                    queryString = "select top 1 ImportListID from ImportList order by ImportListID desc";
                    string importID;
                    using (SqlCommand cmd = new SqlCommand(queryString, connect))
                    {
                        importID = cmd.ExecuteScalar().ToString();
                    }

                    foreach (ListViewItem product in prodListView.Items)
                    {
                        queryString = "insert into Imported (ImportListID, ProdID, ProdName, Unit, UnitPrice, Quantity, Discount) " +
                            "values (@importListId, @prodId, @prodName, @unit, @unitPrice, @quantity, @discount)";
                        using (SqlCommand cmd = new SqlCommand(queryString, connect))
                        {
                            cmd.Parameters.Add("@importListId", SqlDbType.NVarChar).Value = importID;
                        ...
                        queryString = "update Product set Quantity = @quantity, UnitPrice = @price where ProdID = @prodId";
                        using (SqlCommand cmd = new SqlCommand(queryString, connect))
                        {
                            int quantity = 0;
                            foreach (DataRow row in prodData.Rows)
                            ...
                            cmd.Parameters.Add("@price", SqlDbType.Money).Value = Decimal.Parse(product.SubItems[3].Text);
                            cmd.Parameters.Add("@prodId", SqlDbType.NVarChar).Value = product.SubItems[0].Text;
                            cmd.ExecuteNonQuery();
                        }
